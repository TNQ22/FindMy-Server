...
class FindMyLocationReport {
  ...
  Location get location => Location(latitude ?? 0.0, longitude ?? 0.0);
  ...
  Future<void> decrypt() async {
    try {
      ...
      if (isEncrypted() && base64privateKey != null && id != null && result != null && result["payload"] != null) {
        final unixTimestampInMillis = result["datePublished"] ?? DateTime.now().millisecondsSinceEpoch;
        final datePublished =
            DateTime.fromMillisecondsSinceEpoch(unixTimestampInMillis);
        FindMyReport report = FindMyReport(datePublished,
            base64Decode(result["payload"]), id!, result["statusCode"] ?? 200);

        FindMyLocationReport decryptedReport =
            await DecryptReports.decryptReport(
                report, base64Decode(base64privateKey!));
        if (decryptedReport.latitude != null) {
          latitude = correctCoordinate(decryptedReport.latitude!, 90);
        }
        if (decryptedReport.longitude != null) {
      ...
      logger.e('Failed to decrypt report: $e');
    }
  }
