...
class Accessory {
  ...
  Accessory.fromJson(Map<String, dynamic> json)
      : id = json['id']?.toString() ?? '',
        ...
        hashedPublicKey = json['hashedPublicKey']?.toString() ?? '',
        ...
        isActive = json['isDeployed'] ?? json['isActive'] ?? true,
        _icon = json['icon']?.toString() ?? 'tag',
  ...
  static Color _parseColor(dynamic val) {
    if (val == null) return Colors.blue;
    try {
      if (val is int) return Color(val);
      String s = val.toString();
      if (s.length >= 8) {
        return Color(int.parse(s.substring(0, 8), radix: 16));
      }
    } catch (_) {}
    return Colors.blue;
  }

  static AccessoryBatteryStatus? _parseBatteryStatus(dynamic val) {
    if (val == null) return null;
    try {
      return AccessoryBatteryStatus.values.byName(val.toString());
    } catch (_) {}
    return null;
  }

  static Map<String, dynamic> _parseHashesWithTS(dynamic val) {
  ...
  static List<String> _parseAdditionalKeys(dynamic val) {
    if (val == null) return [];
    try {
      if (val is List) {
  ...
  Map<String, dynamic> toJson() {
    ...
    return {
      'id': id.isNotEmpty ? id : hashedPublicKey,
      'name': name,
      'hashedPublicKey': hashedPublicKey,
